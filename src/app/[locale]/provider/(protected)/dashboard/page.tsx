...
export default async function ProviderDashboardPage() {
  ...
  // Fetch this provider's bookings via their provider_services
  ...
  const [{ data: allBookings }, { data: upcoming }] = await Promise.all([
    ids.length
      ? db.from("bookings").select("total_amount, provider_amount, status, booking_date, provider_service_id").in("provider_service_id", ids).gte("booking_date", monthStart)
      : { data: [] },
    ids.length
      ? db.from("bookings").select("*").in("provider_service_id", ids).gte("booking_date", today).order("booking_date").limit(5)
      : { data: [] },
  ]);
  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="mb-8">
        <h1 className="text-2xl font-semibold text-stone-900">Welcome back, {provider.name.split(" ")[0]}</h1>
        <p className="text-sm text-stone-400 mt-0.5">Here&apos;s what&apos;s coming up</p>
      </div>
      ...
      <Card className="border-stone-200">
        <CardHeader className="px-6 pt-5 pb-3">
          <CardTitle className="text-sm font-medium text-stone-700">Upcoming Bookings</CardTitle>
        </CardHeader>
                ...
                return (
                  <div key={b.id} className="flex items-center justify-between py-2 border-b border-stone-50 last:border-0">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-stone-500 mt-1">€{b.provider_amount?.toFixed(2)}</p>
                    </div>
                  </div>
                );
              })}
